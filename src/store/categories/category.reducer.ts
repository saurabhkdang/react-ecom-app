...
export const categoriesReducer = (state = CATEGORIES_INITIAL_STATE, action = {} as AnyAction):CategoriesState => {
    ...
    return state;
    /* switch(action.type) {
        case CATEGORIES_ACTION_TYPES.FETECH_CATEGORIES_START:
            return {...state, isLoading:true};
        case CATEGORIES_ACTION_TYPES.FETECH_CATEGORIES_SUCCESS:
            return {...state, categories:action.payload,isLoading:false};
        case CATEGORIES_ACTION_TYPES.FETECH_CATEGORIES_FAILED:
            return {...state, error:action.payload, isLoading: false};
        default:
            return state;
    } */
